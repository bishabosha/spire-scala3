...
trait CRingFunctions[C[T] <: CRing[T]] extends CRigFunctions[C] {
  ...
  def negate[@sp(Int, Long, Float, Double) A](x: A)(using ev: C[A]): A =
    ev.negate(x)

  def minus[@sp(Int, Long, Float, Double) A](x: A, y: A)(using ev: C[A]): A =
    ev.minus(x, y)

  def fromInt[@sp(Int, Long, Float, Double) A](n: Int)(using ev: C[A]): A =
    ev.fromInt(n)

  def fromBigInt[@sp(Int, Long, Float, Double) A](n: BigInt)(using ev: C[A]): A =
    ev.fromBigInt(n)

}

object CRing extends CRingFunctions[CRing] {
  inline def apply[A](using CRing[A]) = summon[CRing[A]]
}
